        ...
        <AnimatedBorder animationMode="auto-rotate" animationSpeed={6} borderRadius={12} borderWidth={1.5} style={{ marginBottom: 48 }}>
          <div style={{
            ...
            <div>
              ...
              <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Nome ou palavra-chave..." style={{ ...selectStyle, backgroundImage: 'none', paddingRight: 16 }} />
            </div>
            <button onClick={() => { setBusca(''); setEstado('Todos os estados'); setEspecialidade('Todas as especialidades') }}
              style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, color: 'rgba(255,255,255,0.4)', fontSize: 13, padding: '12px 20px', cursor: 'pointer', whiteSpace: 'nowrap', transition: 'all 0.2s' }}
              ...
              <div style={{ background: '#0d0d0d', borderRadius: 12, overflow: 'hidden', cursor: 'pointer' }} onClick={() => setSelected(f)}>
                <div style={{ position: 'relative', height: 200, background: 'rgba(255,255,255,0.03)', overflow: 'hidden' }}>
                  {f.foto_url ? (
                    ...
                    <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 48, color: 'var(--verde)', fontFamily: 'var(--font-display)' }}>
                      {f.nome.split(' ').slice(0, 2).map(n => n[0]).join('')}
                    </div>
                  )}
                  <div style={{ position: 'absolute', bottom: 12, left: 12, background: cor(f.area).bg, border: `1px solid ${cor(f.area).border}`, backdropFilter: 'blur(8px)', borderRadius: 100, padding: '4px 12px', fontSize: 11, color: cor(f.area).text, fontWeight: 500 }}>
                    {f.area}
                  </div>
                </div>
                <div style={{ padding: '20px 20px 16px' }}>
                  ...
                  <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', lineHeight: 1.6, marginBottom: 16 }}>
                    {f.bio.length > 110 ? f.bio.slice(0, 110) + '...' : f.bio}
                  </p>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <a href={`https://instagram.com/${f.instagram}`} target="_blank" rel="noopener" onClick={e => e.stopPropagation()}
                      style={{ flex: 1, textAlign: 'center', padding: '8px', background: 'var(--verde)', color: '#000', borderRadius: 6, fontSize: 12, fontWeight: 500, textDecoration: 'none' }}>
                      Contatar
                    </a>
                    <button onClick={e => { e.stopPropagation(); setSelected(f) }}
                      style={{ flex: 1, padding: '8px', background: 'transparent', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 6, fontSize: 12, color: 'rgba(255,255,255,0.5)', cursor: 'pointer' }}>
                      Ver perfil
                    </button>
                  </div>
                </div>
          ...
          <AnimatedBorder animationMode="auto-rotate" animationSpeed={4} borderRadius={16} borderWidth={1.5} style={{ maxWidth: 580, width: '100%' }}>
            <div onClick={e => e.stopPropagation()} style={{ background: '#111', borderRadius: 16, overflow: 'hidden', position: 'relative' }}>
              ...
              <div style={{ padding: '0 32px 32px' }}>
                <h3 style={{ fontSize: 22, fontWeight: 500, color: '#fff', marginBottom: 4 }}>{selected.nome}</h3>
                ...
                <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.6)', lineHeight: 1.8, marginBottom: 28 }}>{selected.bio}</p>
                <div style={{ display: 'flex', gap: 12 }}>
                  <a href={`https://instagram.com/${selected.instagram}`} target="_blank" rel="noopener" style={{ flex: 1, textAlign: 'center', padding: '12px', background: 'var(--verde)', color: '#000', borderRadius: 8, fontSize: 14, fontWeight: 500, textDecoration: 'none' }}>Contatar via Instagram ↗</a>
                  <a href={`mailto:anne@institutoamplifica.com?subject=Interesse em entrevistar ${selected.nome}`} style={{ flex: 1, textAlign: 'center', padding: '12px', background: 'transparent', color: '#fff', borderRadius: 8, fontSize: 14, border: '1px solid rgba(255,255,255,0.15)', textDecoration: 'none' }}>Solicitar via Amplifica</a>
                </div>
              </div>
            </div>
          </AnimatedBorder>
