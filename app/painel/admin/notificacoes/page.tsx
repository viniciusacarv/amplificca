// app/painel/admin/notificacoes/page.tsx
// Caixa de entrada do admin — novas submissões e alertas do sistema

import { createClient } from '@/lib/supabase-server'
import { redirect } from 'next/navigation'
import Link from 'next/link'
import { marcarNotificacoesAdminLidas } from '../imprensa/actions'
...
export default async function AdminNotificacoesPage() {
  ...
  const { data: notificacoes } = await supabase
    .from('notificacoes')
    .select('*, submissoes(titulo, tipo, fellows(nome))')
    .eq('is_admin', true)
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {naoLidas > 0 && (
          <form action={marcarNotificacoesAdminLidas}>
            <button
              type="submit"
        ...
        {!notificacoes || notificacoes.length === 0 ? (
          ...
          <div className="divide-y divide-gray-800">
            {(notificacoes as any[]).map((notif) => {
              const href = notif.submissao_id
                ? `/painel/admin/imprensa/${notif.submissao_id}`
                : null
              ...
              return (
                <Wrapper key={notif.id}>
                  <div className={`flex items-start gap-4 px-6 py-4 ${!notif.lida ? 'bg-emerald-500/5' : ''}`}>
                    {/* Ícone */}
                    <div className="flex-shrink-0 w-9 h-9 rounded-full bg-emerald-500/15 border border-emerald-500/20 flex items-center justify-center text-base mt-0.5">
                      📬
                    </div>
                    {/* Conteúdo */}
                    <div className="flex-1 min-w-0">
                      ...
                      {href && (
                        <p className="text-xs text-emerald-400 mt-1.5">Ver submissão →</p>
                      )}
                    </div>
                  </div>
                </Wrapper>
