// Tabela de fellows + status da cobrança do mês selecionado.

import { atualizarStatusCobranca } from '../actions'
...
function StatusPill({ status }: { status: Cobranca['status'] | 'sem_cobranca' | 'bolsista' }) {
  const map = {
            ...
            {fellows.length === 0 && (
              <tr>
                <td colSpan={5} className="px-5 py-10 text-center text-gray-500">
                  Nenhum fellow corresponde ao filtro.
                </td>
              </tr>
            )}
            {fellows.map((f) => {
              ...
              const status: Cobranca['status'] | 'sem_cobranca' | 'bolsista' = isBolsista
                ? 'bolsista'
                : c
                ? c.status
                : 'sem_cobranca'
              ...
              return (
                <tr key={f.id} className="hover:bg-gray-900/40">
                  ...
                  <td className="px-3 py-3 text-gray-300">
                    {f.tipo_financiamento === 'autofinanciado' && (
                      <span className="text-xs text-gray-300">Autofinanciado</span>
                    )}
                    {f.tipo_financiamento === 'bolsista' && (
                      <span className="text-xs text-gray-300">{f.bolsa_origem ?? 'Bolsista'}</span>
                    )}
                  ...
                  <td className="px-3 py-3 text-right tabular-nums text-gray-300">
                    {isBolsista ? '—' : c ? brl(Number(c.valor)) : '—'}
                  </td>
                  <td className="px-5 py-3 text-right">
                    {!isBolsista && c && (
                      <div className="inline-flex gap-1">
                        {(['pago', 'pendente', 'inadimplente'] as const)
                          .filter((s) => s !== c.status)
                          .map((s) => (
                            <form key={s} action={atualizarStatusCobranca}>
                              ...
                              <button
                                ...
                                title={`Marcar como ${s}`}
                              >
                                {s === 'pago' ? '✓ Pago' : s === 'pendente' ? '↻ Pendente' : '⚠ Inadimpl.'}
                              </button>
                            </form>
                          ))}
                      </div>
